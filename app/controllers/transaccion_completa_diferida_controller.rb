...
class TransaccionCompletaDiferidaController < ApplicationController

  PRODUCT = "Transacción completa diferida".freeze
  ...
  def installments
    ...
    begin
      ...
      logger.error("Error en Transacción Completa diferida - Installments: #{e.message}")
      render ERROR_PAGE, locals: { error: e.message }
    end
  end
...
puts "Respuesta de commit: #{resp.inspect}"
      ...
      @request_data = req
      @response_data = resp.with_indifferent_access

      render 'commit'

    rescue StandardError => e
      logger.error("Error en Transacción Completa diferida - Commit: #{e.message}")
      render ERROR_PAGE, locals: { error: e.message }
    end
  end

  def capture
    req = params.permit(:token, :buy_order, :authorization_code, :amount)
    ...
    begin
      ...
      logger.error("Error en Transacción Completa diferida - capture: #{e.message}")
      render ERROR_PAGE, locals: { error: e.message }
    end
  end
  ...
  def status
    ...
    begin
      ...
      logger.error("Error en Transacción Completa diferida - Status: #{e.message}")
      render ERROR_PAGE, locals: { error: e.message }
    end
  end
  ...
  def refund
    ...
    begin
      ...
      logger.error("Error en Transacción Completa diferida - Refund: #{e.message}")
      render ERROR_PAGE, locals: { error: e.message }
    end
  end
  ...
  def set_transbank_transaction
    ...
    commerce_code = ::Transbank::Common::IntegrationCommerceCodes::TRANSACCION_COMPLETA_DEFERRED
